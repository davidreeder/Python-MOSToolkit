...
soundsAndSequences_version  = "0.2"   //RELEASE
...
//
// Only required when CMIX build does NOT enable OSC.
//
...
//--------------------------- -o-
// See classes/MOSMusic.Ornaments for full description of OSC inputs.
...
list  lowSequenceSound()
{
    ...
    dur   = 3.5
    amp   = 50000         // Use when cmixBuildEnablesOSC is false.
    note  = odFirst.o[0]
    pan   = 0

    if (cmixBuildEnablesOSC) {
        amp = 20000       // Less gain needed because array of parametric EQs are disabled.
    }
    ...
    if  (len(od) == 1)  
    {
        ...
        bpm          = odSecond.o[0] * 4
        subdivision  = odSecond.o[1]
        orn          = odSecond.o[2]   // ornament
        ...
        //
        if         ("sixteenthLeadIn" == odSecond.label)  {
        ...
        } else if  ("sixteenthPop" == odSecond.label)  {
        ...
        } else if  ("sixteenthTripletTurnaround" == odSecond.label)  {
            ...
            FATAL("UNRECOGNIZED ornament.", {odSecond.label})
        }
    }
...
//--------------------------- -o-
// Looch sample library.  (Indices into loocher.aiff.) --
...
list  main(list arbitraryOSCDataFromPython)
{
    ...
    //
    if       ("/lowSequence"   == odFirst.label)  { lowSequenceSound() } 
    else if  ("/highSequence"  == odFirst.label)  { highSequenceSound() } 
    else if  ("/boom"          == odFirst.label)  { boomSound() } 
    else if  ("/kapow"         == odFirst.label)  { kapowSound() } 

    else if  ("/resetScore"    == odFirst.label)
        {
            if (cmixBuildEnablesOSC) {
                hsState = UNDEFINED
            } else {
